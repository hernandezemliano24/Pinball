...
## Version 12 verified launcher and physics

- Dedicated shooter phase prevents gravity and walls from weakening the launch.
- Launch power is strong enough to reach the shooter exit every time.
- The ball is centered inside the tunnel to prevent locked starting positions.
- One-time handoff places the ball safely inside open playfield space.
- Normal physics begin only after the handoff.
- Solid rails use smaller movement substeps and thicker collision boundaries.
- Flippers were strengthened while retaining a safe maximum speed.
- Added cache version `v=12`.
